...
public:
	...
	Rjecnik<int, int> operator() (int from, int to)
	{
		Rjecnik<int, int> rjecnik;
		for (size_t i = from; i <= to; i++)
			rjecnik.AddElement(_elementi1[i], _elementi2[2]);
		return rjecnik;
	}
...
public:
	...
	friend ostream& operator<< (ostream& COUT, const ZadovoljstvoKupca& obj)
	{
		...
		{
			cout << "\t" << obj._komentariKarakteristika.getElement1(i) << " - " << obj._komentariKarakteristika.getElement2(i);
...
public:
	...
	void SacuvajBodove()
	{
		string ekstenzija = ".txt";
		string naziv = _emailAdresa + ekstenzija;
		ofstream fajl(naziv);
...
void main()
{
 ...
 // */
   ...
   */
	...
	cout << endl << endl << "Rezultat pretrage -> " << crt << osteceniProizvodi << crt;
