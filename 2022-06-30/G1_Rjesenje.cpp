...
char* GetNizKaraktera(const char* sadrzaj)
{
    if (sadrzaj == nullptr)
        return nullptr;
    char* novi = new char[strlen(sadrzaj) + 1];
    strcpy_s(novi, strlen(sadrzaj) + 1, sadrzaj);
    return novi;
}
...
public:
    ...
    Kolekcija<int, int> InsertAt(int lokacija, T1 e1, T2 e2)
    {
        T1* temp1 = _elementi1;
        T2* temp2 = _elementi2;
...
public:
    ...
    friend ostream& operator<< (ostream& COUT, const Komentar& obj)
    {
        ...
        {
            COUT << "\t" << IspisKriterija[i] << " (" << obj._ocjeneKriterija->getElement2(i) << ")";
            if (i != obj._ocjeneKriterija->getTrenutno() - 1)
                COUT << endl;
        }
        COUT << crt << "Prosjecna ocjena -> " << obj.ProsjecnaOcjena();
        return COUT;
    }
};
...
public:
    ...
    {
        COUT << obj._imePrezime << " " << obj._emailAdresa << " " << obj._brojPasosa;
        return COUT;
    }
...
public:
    Rezervacija(Datum& OD, Datum& DO, Gost& gost)
        :_OD(OD), _DO(DO)
    {
   ...
   */
    pair<int, int> GetBrojZnakova(const char* putanja, const char* trazeniZnakovi)
    {
        ...
        while (fajl.get(karakter))
        {
            ...
            for (size_t i = 0; i < strlen(trazeniZnakovi); i++)
                if (karakter == trazeniZnakovi[i])
                    trazenihKaraktera++;
        }

        fajl.close();
        return { totalKaraktera, trazenihKaraktera };
    }
