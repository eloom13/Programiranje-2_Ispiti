...
bool ValidirajLozinku(string lozinka)
{
	...
	string Broj = "(?=.*[0-9])";
	string Specijalni = "(?=.*\\W)";
	regex Provjera(Znakovi + VelikaSlova + MalaSlova + Broj + Specijalni);
	return regex_search(lozinka, Provjera);
}
...
public:
	...
	Kolekcija& operator=(const Kolekcija& obj)
	{
		if (this != &obj)
		{
			...
			{
				_elementi1[i] = new T1(*obj._elementi1[i]);
				_elementi2[i] = new T1(*obj._elementi1[i]);
			}
		}
		return *this;
	}
	...
	void AddElement(T1 e1, T2 e2)
	{
		...
		if (*_trenutno >= max)
			throw exception("Dostignut je max!");
...
public:
	...
	*/
	...
	friend ostream& operator<< (ostream& COUT, const Tehnika& obj)
	{
		...
		COUT << "\tProsjecna Ocjena -> " << obj.ProsjecnaOcjenaZaTehniku();
		COUT << crt;
		return COUT;
	}
};
...
public:
	...
	{
		COUT << "IME I PREZIME" << obj._imePrezime << " EMAIL ADRESA" << obj._emailAdresa << "LOZINKA: " << obj._lozinka << endl;
		return COUT;
	}
};
...
public:
	...
	*/
	...
	void PosaljiMail(Tehnika tehnika, Pojas pojas, float ProsjekPojasa)
	{
		...
		cout << "Dosadasnji uspjeh na pojasu " << pojas << "iznosi " << ProsjekPojasa << ", ukupni uspjeh (prosjek ocjena) na svim pojasevima iznosi " << UkupniPojasevi();
		cout << "\nPozdrav" << endl;
		cout << "KARATE Team" << crt2;
		mtx.unlock();
	}
	...
	*/
	bool AddTehniku(Pojas pojas, Tehnika tehnika)
	{
		for (int i = 0; i < _polozeniPojasevi.size(); i++)
		{
			///      NARADZASTI  > ZUTOG                                  broj tehnika manji od 3 ili prosjek pojasa manji od 3.5 
		...
		Polaganje polag(pojas);  // Polaganje sa ZUTI pojas
		polag.GetTehnike().push_back(new Tehnika(tehnika));  // ZUTI Pojas sa proslijedjenom tehnikom
		_polozeniPojasevi.push_back(polag);
		thread t2(&KaratePolaznik::PosaljiMail, this, tehnika, pojas, polag.ProsjekPojasa());
		t2.join();
		return true;
	}
	...
	*/
	...
	if (jasminPolaznik != nullptr)
	{
		if (jasminPolaznik->AddTehniku(ZUTI, gyaku_zuki))
			cout << endl << "Tehnika uspjesno dodana!" << crt;
